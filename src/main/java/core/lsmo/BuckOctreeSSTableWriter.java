 ...
 * @author xiafan
 */
public class BuckOctreeSSTableWriter extends ISSTableWriter {
	private static final Logger logger = Logger.getLogger(BuckOctreeSSTableWriter.class);
	GroupByKeyIterator<WritableComparableKey, IOctreeIterator> iter;

	//file that stores non-empty octants
	FileOutputStream dataFileOs;
	DataOutputStream dataDos;
	
	//files that stores sentinel octants
	 ...
	 *            配置信息
	 */
	public BuckOctreeSSTableWriter(SSTableMeta meta, List<ISSTableReader> tables, Configuration conf) {
		super(meta, conf);
		...
		try {
			indexHelper = (IndexHelper) Class.forName(conf.getIndexHelper()).getConstructor(ISSTableWriter.class, Configuration.class)
	...
	public BuckOctreeSSTableWriter(List<IMemTable> tables, Configuration conf) {
		super(null, conf);
	...
	public void write() throws IOException {
		while (iter.hasNext()) {
			...
			// setup meta values
			indexHelper.startPostingList(entry.getKey(), dataBuck.blockIdx());
			...
			indexHelper.endPostingList(dataBuck.blockIdx());// end a new posting list
		}
	}
