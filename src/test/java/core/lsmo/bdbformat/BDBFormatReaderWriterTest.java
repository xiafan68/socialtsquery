...
public class BDBFormatReaderWriterTest extends BDBFormatCommon {
	...
	public void test() throws IOException {
		...
		ISSTableReader reader = OctreeBDBFormatLSMTFactory.INSTANCE.newSSTableReader(index, meta);
		reader.init();
	...
	public void compact() throws IOException {
		...
		ISSTableReader reader1 = OctreeBDBFormatLSMTFactory.INSTANCE.newSSTableReader(index, meta1);
		reader1.init();
		...
		ISSTableWriter writer = OctreeBDBFormatLSMTFactory.INSTANCE.newSSTableWriterForCompaction(meta2, readers,
				conf);
		writer.open(conf.getIndexDir());
		writer.write();
		writer.close();

		ISSTableReader reader2 = OctreeBDBFormatLSMTFactory.INSTANCE.newSSTableReader(index, meta2);
		reader2.init();
		int dataNum = 0;
		for (Object obj : termCounts.entrySet()) {
			if (dataNum++ % 100 == 0) {
